...
class JDETracker(object):
    def __init__(self, opt, frame_rate=30):
        ...
        self.mean = np.array(opt.mean, dtype=float32).reshape(1, 1, 3)
        self.std = np.array(opt.std, dtype=float32).reshape(1, 1, 3)
    ...
    def post_process(self, dets, meta):
        ...
        for j in range(1, self.opt.num_classes + 1):
            dets[0][j] = np.array(dets[0][j], dtype=float32).reshape(-1, 5)
        return dets[0]
    ...
    def merge_outputs(self, detections):
        ...
        for j in range(1, self.opt.num_classes + 1):
            results[j] = np.concatenate(
                [detection[j] for detection in detections], axis=0).astype(float32)
    ...
    def update(self, im_blob, img0):
        ...
        c = np.array([width / 2., height / 2.], dtype=float32)
...
def remove_fp_stracks(stracksa, n_frame=10):
    ...
    for t in stracksa:
        ...
        score_5 = np.array(score_5, dtype=float32)
        index = score_5 < 0.45
        num = np.sum(index)
        if num < n_frame:
            remain.append(t)
